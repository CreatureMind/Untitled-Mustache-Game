...
public class Level_Button : MonoBehaviour
{
    ...
    public void InitializeLevelButton(Level_Button_Data levelButtonData, int starsEarned = 0,
        LevelStateType levelState = LevelStateType.Locked)
    {
    ...
    {
        mapImagePath = $"{Profile_Menu.ProfilesPath}/{Game_Manager.ActiveProfile.nickname}/ScreenShots/Level_{levelButtonData.levelIndex}/map.png";
        if (System.IO.File.Exists(mapImagePath))
        {
            ...
            mapImage.sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height),
